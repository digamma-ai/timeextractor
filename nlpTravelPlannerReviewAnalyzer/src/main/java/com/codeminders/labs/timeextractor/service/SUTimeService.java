...
import com.codeminders.labs.timeextractor.rest.entities.Annotation2HtmlTemporalConversion;
...
public class SUTimeService {
    ...
    private Annotation2HtmlTemporalConversion converter;

    {
        converter = new Annotation2HtmlTemporalConversion();
    }
    ...
    public List<TemporalExtraction> extractDatesAndTimeFromText(String text, String date) {
        if (text == null) {
            return null;
        }
        ...
        List<TemporalExtraction> extracted = annotation.get(TemporalExtractionAnnotation.class);
        if (extracted != null) {
    ...
    public List<AnnotationInterval> getAllAnotations(List<TemporalExtraction> annotations) {
    ...
    public Map<String, List<TemporalExtraction>> extractDatesAndTimeFromText(List<BaseText> baseTexts) {
        Map<String, List<TemporalExtraction>> map = new HashMap<String, List<TemporalExtraction>>();
        for (BaseText baseText : baseTexts) {
            List<TemporalExtraction> results = extractDatesAndTimeFromText(baseText.getText(), baseText.getDate());
    ...
    public Map<String, List<AnnotationIntervalHtml>> extractDatesAndTimeFromHtml(String html) {
        ...
        for (HtmlElement htmlElement : htmlElements) {
            List<TemporalExtraction> results = extractDatesAndTimeFromText(htmlElement.getExtractedText(), null);
            if (results.size() > 0) {
     ...
     */
    private Map<String, List<AnnotationIntervalHtml>> getAnnotationIntervalsForHtml(Map<HtmlElement, List<TemporalExtraction>> map) {
        Map<String, List<AnnotationIntervalHtml>> resultMap = new HashMap<String, List<AnnotationIntervalHtml>>();
        int count = 1;
        for (Map.Entry<HtmlElement, List<TemporalExtraction>> entry : map.entrySet()) {
            ...
            for (TemporalExtraction annotation : annotations) {
                ...
                List<HtmlTemporal> extracted = converter.convert(annotation.getTemporal());
    ...
    public Map<String, List<AnnotationInterval>> getAllAnnotations(Map<String, List<TemporalExtraction>> extractedTemporal) {
