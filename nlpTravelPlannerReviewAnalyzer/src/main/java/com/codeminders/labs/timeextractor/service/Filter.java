...
public class Filter {
	...
	private void filterLastCharacter(TemporalExtraction extraction) {
		char lastCharacter = extraction.getTemporalExpression().charAt(
				extraction.getTemporalExpression().length() - 1);
		if (lastCharacter == '.' || lastCharacter == ','
				|| lastCharacter == ' ' || lastCharacter == ';'
				|| lastCharacter == '!' || lastCharacter == '?') {
			int to = extraction.getToPosition() - 1;
			String finalString = extraction.getTemporalExpression().substring(
					0, extraction.getTemporalExpression().length() - 1);
	...
	private void filterFirstCharacter(TemporalExtraction extraction) {
		...
		if (lastCharacter == ' ') {
			...
			String finalString = extraction.getTemporalExpression().substring(
					1, extraction.getTemporalExpression().length());
			extraction.setTemporalExpression(finalString);
