...
public class TimeExtractorRestService {
	...
	public Response getAllAnnotationsForPlainText(String text,
			@QueryParam("date") String date, @QueryParam("locale") String locale) {
		List<CoreMap> result = sutimeService.extractDatesAndTimeFromText(text,
				date);
		List<AnnotationInterval> intervals = sutimeService
				.getAllAnotations(result);
	...
	@Produces(MediaType.APPLICATION_JSON)
	public Response getAllAnnotationsForMultipleTexts(JSONArray jsonArray, @QueryParam("locale") String locale)
