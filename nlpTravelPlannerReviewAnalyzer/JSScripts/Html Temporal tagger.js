		...
		if (current_tag.length == 1) {
			...
			if ($(base_tag).children().length < 2) {
				...
				for (var i = 0; i < current_tag.length; i++) {
					...
					var newText = baseText.substring(0, from)
							+ "<span data-tooltip aria-haspopup=\"true\" class=\"has-tip highlight\" title=\""
							+ JSON.stringify(current_tag[i].extractedTemporal)
									.replace(/"/g, '\'') + "\">"
							+ baseText.substring(from, to) + "</span>"
							+ baseText.substring(to, baseText.length);
					baseText = newText;
					count = count
							+ ("<span data-tooltip aria-haspopup=\"true\" class=\"has-tip highlight\" title=\""
									+ JSON.stringify(
											current_tag[i].extractedTemporal)
											.replace(/"/g, '\'') + "\">").length
							+ "</span>".length;
				}
				...
				var tag = $(current_tag[0].tag).filter(
						function() {
							return $(this).text().toLowerCase().trim() === text
									.toLowerCase().trim();
						})
				$(tag).text('');
				$(tag).append(newText);
	...
	for (var j = 0; j < tags.length; j++) {
		...
		if ($(tags[j].tag).length > 1) {
			...
			for (var k = 0; k < tagss.length; k++) {
				...
				var newText = text.substring(0, tags[j].from)
						+ "<span data-tooltip aria-haspopup=\"true\" class=\"has-tip highlight\" title=\""
						+ JSON.stringify(tags[j].extractedTemporal).replace(
								/"/g, '\'') + "\">"
						+ text.substring(tags[j].from, tags[j].to) + "</span>"
						+ text.substring(tags[j].to, text.length);
				$(tagss[k]).text('');
			...
			var newText = text.substring(0, tags[j].from)
					+ "<span data-tooltip aria-haspopup=\"true\" class=\"has-tip highlight\" title=\""
					+ JSON.stringify(tags[j].extractedTemporal).replace(/"/g,
							'\'') + "\">"
					+ text.substring(tags[j].from, tags[j].to) + "</span>"
					+ text.substring(tags[j].to, text.length);
			$(tags[j].tag).text('');
