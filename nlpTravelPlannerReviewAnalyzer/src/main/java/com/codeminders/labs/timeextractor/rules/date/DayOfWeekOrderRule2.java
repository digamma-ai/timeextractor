...
public class DayOfWeekOrderRule2 extends BaseRule {
    ...
    public static String rule = "\\b(the)?[\\s]*" + DAY_OF_MONTH + "(th|st|nd)?[\\s]*(of)?[\\s]*" + "(" + DAY_OF_WEEK + "|" + DAY_OF_WEEK_EASY + ")" + "[s]?[\\s]*(of (the)? month)?";
    private String extractedText;
    protected Locale locale = Locale.US;
    protected double confidence = 0.83;

    public DayOfWeekOrderRule2(String extractedText) {
    ...
    public List<Temporal> getTemporal() {
        ...
        while (matcher.find()) {
            dayOfWeek = TemporalBasicCaseParser.getDayOfWeek((matcher.group(5)));
