...
{ 
 ...
 var highlight = function(html, data){
     ...
     for (var property in data) {
         ...
         if (current_tag.length==1){
            ...
            var text = $.trim($(base_tag).clone().children().remove().end().text());
            text = text.replace(/\s+/g, " ");
            var temporal = (text.substring(current_tag[0].from, current_tag[0].to)); 
            ...
            var base_tag =  $(html.substring(current_tag[j].htmlTagFrom,current_tag[j].htmlTagTo));
            var text = $.trim($(base_tag).clone().children().remove().end().text());
            text = text.replace(/\s+/g, " ");
            var tag = $(current_tag[0].tag).filter(function(){ return $(this).text().toLowerCase() === $(base_tag).text().toLowerCase();})
            var temporal = (text.substring(current_tag[j].from, current_tag[j].to)); 
 			var result = {
             'tag':tag,
             'temporal': temporal,
     ...
     for (var j = 0; j < tags.length; j++) {
     	$(tags[j].tag).html(function(i, v) {
   			 return v.replace(tags[j].temporal, "<span data-tooltip aria-haspopup=\"true\" class=\"has-tip highlight\" title=\"" + JSON.stringify(tags[j].extractedTemporal)
									.replace(/"/g, '\'') + "\">"+tags[j].temporal+"</span>");    
});
 ...
 jQuery.fn.textWalk = function( fn ) {
    ...
    function jwalk() {
        ...
        if( nn === '#text' ) {
            fn.call( this );
        } else if( this.nodeType === 1 && this.childNodes && this.childNodes[0] && nn !== 'textarea' ) {
            $(this).contents().each( jwalk );
        }
    }
    return this;
